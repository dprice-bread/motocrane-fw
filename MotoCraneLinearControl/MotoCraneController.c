...
// Speed parameters
# define SPEED_SHIFT	4 // Bit shift by 4 (x16) to scale between input speed values (out of 100) and output speed values (out of 3200)
# define DEADZONE		5 // -5% <-> -0% & +0% <-> 5% treated as a deadzone and skipped over
	...
	while(1) {
		...
		if (motorCmdReady) { // At least 1 command has been received and is waiting for comm to the motor
			...
			if ((RXNum != 0) | (currentOutgoSpeed != 0)) { // But only do so if have real data and aren't here due to a timeout (or we are here due to timeout but current speed is nonzero)
				...
				TXValue[0] = (char) currentOutgoSpeed & 0x00FF;
