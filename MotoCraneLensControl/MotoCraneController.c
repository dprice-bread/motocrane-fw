...
// Speed parameters
# define SPEED_SHIFT	5 // Bit shift by 5 (x32) to scale between input speed values (out of 100) and output speed values (out of 3200)
# define DEADZONE		1 // -5% <-> -0% & +0% <-> 5% treated as a deadzone and skipped over
...
//const unsigned int SLAVEADDRESS = 0x651f; // For Roll Motor
...
// Encoder variables
char readState; // Current state
char oldState; // Past state
	...
	while(1) {
		...
		if (motorCmdReady) { // At least 1 command has been received and is waiting for comm to the motor
			...
			if ((RXNum != 0) | (currentOutgoSpeed != 0)) { // But only do so if have real data and aren't here due to a timeout (or we are here due to timeout but current speed is nonzero)
				...
				TXValue[0] = (char) currentOutgoSpeed >> SPEED_SHIFT;
				TXValue[1] = (unsigned char) voltage & 0x00FF;
				TXValue[2] = (unsigned char) (rotNumber >> 16) & 0x000000FF;
				TXValue[3] = (unsigned char) (rotNumber >> 8) & 0x000000FF;
...
int readSpeed(void) {
	...
	while(isTransmitting); // while UART transmission is in progress
	delayMilliseconds(3); // Delay 3 ms
	Pspeed = RXSData[0]; // This is the voltage x 4
	Pspeed = Pspeed + (RXSData[1]<<8);
	return Pspeed;
}
...
__interrupt void PORT1_ISR(void){
	...
	oldState = readState;
	readState = P1IN & (PIN_A|PIN_B);

	// Switches the edge select direction to allow for detection of both edges
	if (P1IFG == PIN_A) {
		P1IES ^= PIN_A;
	} else {
		P1IES ^= PIN_B;
	...
	// Update rotNumber using encoder input
	if (readState == PIN_A) {
		if (oldState == 0x00) {
			rotNumber--;
		} else if (oldState == (PIN_A|PIN_B)) {
			rotNumber++;
		}
		oldState = readState;
	}
